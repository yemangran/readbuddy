...
2. **Storage Separation**: Maintain review states in an independent Local Review Store (`readfrog-reviews.json`), decoupled from vocabulary record contents (`readfrog.json`).
3. **WebDAV Dual Sync**: The WebDAV synchronizer manages two files side-by-side: `readfrog.json` (vocabulary cards) and `readfrog-reviews.json` (review states). Review state conflicts are resolved via Last-Review-Wins (LWW) timestamp merging.
